...
Run all integration tests synchronous can be very slow. The command above can run everything:
```shell
go test -v -timeout 120m ./integration
```

Or you can run only the specific test folder, like the `list` types test
```shell
