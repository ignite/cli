...
Uygulamanızı test etmek için test dosyalarını kodunuza ekleyin.

Test dosyalarını ekledikten sonra, terminalinizle `interchange` dizinine geçin ve ardından çalıştırın:

```
go test -timeout 30s ./x/dex/types
```

### Sipariş Defteri Testleri

types dizininde yeni bir `x/dex/types/order_book_test.go` dosyası oluşturun.

Aşağıdaki testsuite'i ekleyin:
...
Buy Order Book için testleri eklemek üzere `types` dizininde yeni bir `x/dex/types/buy_order_book_test.go` dosyası oluşturun:
...
Yeni bir `x/dex/types/sell_order_book_test.go` dosyasında Satış Siparişleri için yeni bir testsuite oluşturun:
