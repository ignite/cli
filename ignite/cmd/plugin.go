...
func LoadPlugins(ctx context.Context, rootCmd *cobra.Command) error {
	localCfg, _, err := parseLocalConfig(rootCmd)
	...
	globalCfg, _, err := parseGlobalConfig()
	// if binary is run where there is no cfg, don't load
	if err == nil {
...
func parseLocalConfig(rootCmd *cobra.Command) (cfg *pluginsconfig.Config, path string, err error) {
...
func parseGlobalConfig() (cfg *pluginsconfig.Config, path string, err error) {
	globalPath, err := plugin.PluginsPath()
	if err != nil {
		return cfg, globalPath, err
	}

	cfg, err = pluginsconfig.ParseFile(globalPath)
	return cfg, globalPath, err
}
...
func NewPluginAdd() *cobra.Command {
	cmdPluginAdd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			conf, persistPath, err := parseLocalConfig(cmd)
			if err != nil {
				return err
			}
