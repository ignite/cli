...
type Config struct {
	...
	// Extensions holds the list of installed Ignite Extensions.
	// Ignite Extensions are implemented as plugins.
	Extensions []Plugin `yaml:"extensions"`
}
...
type Plugin struct {
	...
	// (default: $HOME/.ignite/ext/extensions.yml) or locally for a chain.
	Global bool `yaml:"-"`
}
...
func (c Config) HasPlugin(path string) bool {
	return slices.ContainsFunc(c.Extensions, func(cp Plugin) bool {
		if cp.HasPath(path) {
			return true
		}
		if cp.IsLocalPath() {
