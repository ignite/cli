...
func ParseGentx(gentx []byte) (info GentxInfo, file []byte, err error) {
	var jsonGentx Gentx
	if err := json.Unmarshal(gentx, &jsonGentx); err != nil {
		return info, gentx, err
	}
	if jsonGentx.Body.Messages == nil {
	...
	info.Memo = jsonGentx.Body.Memo
	info.DelegatorAddress = jsonGentx.Body.Messages[0].DelegatorAddress

	pb := jsonGentx.Body.Messages[0].PubKey.Key
	info.PubKey, err = base64.StdEncoding.DecodeString(pb)
	if err != nil {
		return info, gentx, fmt.Errorf("invalid validator public key %s", err.Error())
	}

	amount, ok := sdkmath.NewIntFromString(jsonGentx.Body.Messages[0].Value.Amount)
	if !ok {
		return info, gentx, errors.New("the self-delegation inside the gentx is invalid")
	}

	info.SelfDelegation = sdk.NewCoin(
		jsonGentx.Body.Messages[0].Value.Denom,
		amount,
	)
