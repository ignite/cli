<template>
  <div class="main">
    <div class="hero">
      ...
      <p>
        Starport has scaffolded and launched a Cosmos blockchain for you. Your
        blockchain has its own tokens, accounts, custom data types and more.
      </p>
    </div>
    ...
    <div class="dashboard -grid-col-3">
      ...
      >
        ...
        <div class="dashboard__card-main">
          ...
          <SpLoaderIconText
            :text="
              `localhost: ${card.port} ${card.id !== 'frontend' ? '' : '→'}`
            "
            :isActive="backendRunningStates[card.id]"
            :link="card.id !== 'frontend' ? '' : appEnv.FRONTEND"
          />
        </div>
        <SpBlockInfoCard v-if="card.id === 'api'" :blockCards="blockCards" />
      </div>
    </div>
    ...
    <div class="-grid-col-3 intro">
      ...
      <div class="intro__main">
        <p>
          ...
          for you. We've also generated an empty module, which you can use to
          start developing your own application features.
        </p>
      </div>
    </div>
    ...
    <div class="tutorials">
        ...
        <div class="-grid-col-3 cards">
          <a
            ...
            :class="[
              ...
              { '-is-dark': card.tagline === 'tutorial' },
            ]"
            :href="card.link"
            target="_blank"
          >
            <div class="text-card__top">
              <span class="text-card__tagline -sp-c-txt-third">{{
                card.tagline
              }}</span>
      ...
      <div class="tutorials__videos -grid-col-3">
        ...
        >
          <img
            class="image-card__img card-wrapper"
            :src="card.imgUrl"
            :alt="card.alt"
          />
          <div class="image-card__text">
            <span class="image-card__text-h1">{{ card.title }}</span>
            <span class="image-card__text-p -sp-c-txt-third">{{
              card.length
            }}</span>
          </div>
        </a>
...
const articles = [
  ...
  },
  {
    ...
    link:
      "https://tutorials.cosmos.network/starport-blog/tutorial/01-index.html",
  },
];
...
const footerBlocks = [
  {
    ...
    link: { text: "Cosmos Discord", url: "https://discord.gg/W8trcGV" },
  },
  {
    title: "Join the community",
    link: { text: "Cosmos SDK Forum", url: "https://forum.cosmos.network" },
  },
  {
    title: "Found an issue?",
    link: {
      text: "Suggest improvements",
      url: "https://github.com/tendermint/starport/issues",
    },
  },
];
      ...
      if (latestBlock) {
        ...
        for (let i = 1; i <= 2; i++) {
          if (parseInt(latestBlock.height) - i > 0) {
            this.insertBlockToStack(
              i,
              this.blockByHeight(parseInt(latestBlock.height) - i)[0]
            );
          } else {
            break;
          }
...
.intro {
  grid-template-areas: "intro-side intro-main intro-main";
}
.intro {
  margin-bottom: 10rem;
}
...
.card-wrapper {
  border-radius: var(--sp-border-radius-primary);
  box-shadow: 0px 0px 1px rgba(0, 0, 0, 0.07), 0px 8px 16px rgba(0, 0, 0, 0.05),
    0px 20px 44px rgba(0, 3, 66, 0.12);
  transition: box-shadow 0.25s ease-out, transform 0.25s ease-out,
    opacity 0.4s ease-out;
}
.card-wrapper:hover {
  box-shadow: 0px 0px 1px rgba(0, 0, 0, 0.07), 0px 12px 24px rgba(0, 0, 0, 0.02),
    0px 30px 66px rgba(0, 3, 66, 0.14);
