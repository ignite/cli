...
func (b *Builder) SimulateProposals(ctx context.Context, chainID string, homeDir string, proposals []int, commandOut io.Writer) (bool, string, error) {
	...
	if err := generateGenesis(ctx, chainInfo, simulatedLaunchInfo, chainHandler); err != nil {
		...
		return false, "genesis cannot be generated from launch information", nil
	}
	...
	if runner.ValidateGenesis(ctx); err != nil {
		return false, "genesis is invalid", nil
	}
	...
	if err := <-exit; err != nil {
		return false, "blockchain cannot start from the generated genesis", nil
	}
