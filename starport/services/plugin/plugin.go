// Package plugin implements starport plugin management.
// A starport plugin is a binary which communicates with the starport binary
...
var (
	...
	pluginsPath = xfilepath.Join(
		chainconfig.ConfigDirPath,
		xfilepath.Path("plugins"),
	)
)

// Plugin represents a starport plugin.
type Plugin struct {
...
// Remote plugins require to be fetched first, in $HOME/.starport/plugins
...
func (p *Plugin) clean() error {
	...
	// Clean the cloneDir, next time the starport command will be invoked, the
