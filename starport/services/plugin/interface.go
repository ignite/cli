...
// A starport plugin must expose a global variable StarportPlugin that
// implements the Plugin interface.
type Interface interface {
	// Commands returns one or multiple commands that will be added to the list
	// of starport commands. It's invoked each time starport is executed, in
	// order to display the list of available commands.
	// Each commands are independant, for nested commands, use the field
	// Command.Commands.
	Commands() []Command
	// Execute will be invoked by starport when a plugin commands is executed.
...
type Command struct {
	...
	// For instance `starport scaffold` will place the command at the
	// `scaffold` command.
	// An empty value is interpreted as `starport` (==root).
