...
func extractCommandPlugins(
	...
	for _, pluginFile := range pluginFiles {
		...
		cmdPlugin, ok := symCmdPlugin.(plugintypes.CmdPlugin)
		if !ok {
			return nil, ErrCommandPluginNotRecognized
		}

		convertedCmdPlugin, ok := cmdPlugin.(CmdPlugin)
		if err := convertedCmdPlugin.Init(ctx); err != nil {
			return nil, err
		}

		for _, command := range convertedCmdPlugin.Registry() {
			if err := validateParentCommand(parentCommand, command.ParentCommand()); err != nil {
				return nil, err
			}
		}

		cmdPlugins = append(cmdPlugins, convertedCmdPlugin)
	}
