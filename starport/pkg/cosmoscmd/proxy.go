...
// startProxyForTunneledPeers hooks the `appd start` command to start an HTTP proxy server and HTTP proxy clients for each node that needs HTTP tunneling.
// HTTP tunneling is activated ** ONLY** if your app's `$APP_HOME/config` dir has an `spn.yml` file and only if this file has `tunneled_peers` field inside with a list of
// tunneled peers/nodes.
//
