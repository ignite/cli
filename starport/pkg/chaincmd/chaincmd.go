...
const (
	...
	goApp          = "go"
	commandTest    = "test"
	optionBenchmem = "-benchmem"
	optionRun      = "-run=^$"
	optionBench    = "-bench=^BenchmarkSimulation"
)
...
func SimappWithInitialBlockHeight(initialBlockHeight string) SimappOption {
	return func(command []string) []string {
		if len(initialBlockHeight) > 0 {
			return append(command, optionSimappInitialBlockHeight, initialBlockHeight)
		}
		return command
	}
}
