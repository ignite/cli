...
export default {
	...
	actions: {
		...
		async QueryBalance({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params: {...key}, query=null }) {
			try {
				...
				let err = new SpVuexError('QueryClient:QueryBalance', 'API Node Unavailable. Could not perform query.')
				err.original = e
				console.error(err)
				return {}
			}
		...
		async QueryAllBalances({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params: {...key}, query=null }) {
			try {
				...
				let err = new SpVuexError('QueryClient:QueryAllBalances', 'API Node Unavailable. Could not perform query.')
				err.original = e
				console.error(err)
				return {}
			}
		...
		async QueryTotalSupply({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params: {...key}, query=null }) {
			try {
				...
				let err = new SpVuexError('QueryClient:QueryTotalSupply', 'API Node Unavailable. Could not perform query.')
				err.original = e
				console.error(err)
				return {}
			}
		...
		async QuerySupplyOf({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params: {...key}, query=null }) {
			try {
				...
				let err = new SpVuexError('QueryClient:QuerySupplyOf', 'API Node Unavailable. Could not perform query.')
				err.original = e
				console.error(err)
				return {}
			}
		...
		async QueryDenomMetadata({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params: {...key}, query=null }) {
			try {
				...
				let err = new SpVuexError('QueryClient:QueryDenomMetadata', 'API Node Unavailable. Could not perform query.')
				err.original = e
				console.error(err)
				return {}
			}
		...
		async QueryDenomsMetadata({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params: {...key}, query=null }) {
			try {
				...
				let err = new SpVuexError('QueryClient:QueryDenomsMetadata', 'API Node Unavailable. Could not perform query.')
				err.original = e
				console.error(err)
				return {}
			}
  ...
  gas: "200000" }, memo})
				...
				if (e == MissingWalletError ) {
					...
					let err = new SpVuexError('TxClient:MsgMultiSend:Send', 'Could not broadcast Tx.')
					err.original = e
					throw err
				}
			}
