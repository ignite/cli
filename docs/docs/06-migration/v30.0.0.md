...
The `cosmossdk.io/x/...` modules move into the main Cosmos SDK module. The `x/circuit` and `x/nft` modules move to `contrib`, and `x/params` and `x/group` are removed:
...
The module config protos for `circuit` and `nft` also move:
...
### Remove x/params and x/group

Cosmos SDK v0.55 removes the `x/params` module. In `app/app.go`, delete the `ParamsKeeper` field, the `GetSubspace` method, the `paramskeeper` and `paramstypes` imports, and the `&app.ParamsKeeper` line in the `depinject.Inject` call. In `app/app_config.go`, delete the `params` module config entry and the related imports. If your chain still mounts the `params` store, delete it in your upgrade handler store upgrades.

The `x/group` module moves to the commercial Cosmos Enterprise offering and is no longer part of the template. In `app/app_config.go`, delete the `group` module config entry, the `groupmodulev1` and `group` imports, and the `group.ModuleName` entries in the end blockers and init genesis order.
