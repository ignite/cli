...
`v0.25.1` changes the location of scaffolded `.proto` files.  Previously, `.proto` files were located in `./proto/{moduleName}/`, 
...
### `v0.25.1` Directory Structure

This example shows a chain that was generated using `ignite` with `v0.25.1` using the following command:
...
import this functionality in their codebase so that their chains can be more easily upgraded and customized.
