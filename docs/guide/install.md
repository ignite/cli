...
You can run [Ignite CLI](https://github.com/tendermint/starport) in a web-based Gitpod IDE or you can install Ignite CLI on your local computer.
...
```bash
curl https://get.ignite.network/cli! | bash
```
...
The Ignite CLI installation requires write permission to the `/usr/local/bin/` directory. If the installation fails because you do not have write permission to `/usr/local/bin/`, run the following command:
...
```bash
sudo curl https://get.ignite.network/cli! | sudo bash
```

## Upgrading your Ignite CLI installation

Before you install a new version of Ignite CLI, remove all existing Starport installations.
...
1. Remove the Ignite binary with `rm $(which ignite)`.
   Depending on your user permissions, run the command with or without `sudo`.
1. Repeat this step until all `ignite` installations are removed from your system.

After all existing Ignite CLI installations are removed, follow the  [Installing Ignite CLI](#installing-starport) instructions.
