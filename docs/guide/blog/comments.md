...
```bash
starport scaffold message createComment id title body
```
...
```go
func (k msgServer) CreateComment(goCtx context.Context, msg *types.MsgCreatePost) (*types.MsgCreatePostResponse, error) {
  // Get the context
...
func (k Keeper) AppendComment(ctx sdk.Context, post types.Post) uint64 {
...
```bash
starport scaffold query post id --response title,body
```
